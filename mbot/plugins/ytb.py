...
from mbot.utils.ytdownloader import robust_download as ytdl_down, thumb_down
from mbot import LOG_GROUP, OWNER_ID, SUDO_USERS, Mbot, AUTH_CHATS
import logging
logger = logging.getLogger(__name__)
import traceback
import json
# Add these imports at the top of your ytb.py file
import logging
from typing import Union, Optional
from mbot.utils.ytdownloader import robust_download, DownloadError
...
async def ytdl_video(path, video_url, id):
    ...
    try:
        return await ytdl_down(ydl_opts, video_url)
    except Exception as e:
        logger.error(f"Video download failed: {str(e)}")
        raise

async def ytdl_down(path: str, video_url: Union[str, list], user_id: Optional[int] = None) -> str:
    """Download YouTube audio with proper error handling"""
    logger = logging.getLogger(__name__)
    try:
        ...
        if isinstance(video_url, list):
            if len(video_url) >= 4:
                ...
                raise DownloadError("Invalid video info format")
        
        # Prepare download options
        out_file = f"{path}/%(title)s.%(ext)s"
        ydl_opts = {
            'format': "bestaudio",
            'default_search': 'ytsearch',
            'noplaylist': True,
        ...
        # FIX: Properly await robust_download
        result = await robust_download(video_url, ydl_opts)
   ...
   # Check Banned Users
    ...
    try:
        ...
        for id in ids:
            ...
            # Updated call to ytdl_down without the id parameter
                fileLink = await ytdl_down(randomdir, id)
                ...
                logger.error(f"Failed to process video {id[0]}: {str(e)}")
                continue
        ...
        if user_id:  # Only try to reply if we have a user_id
            await message.reply_text(YT_RESPONSES.get(user_lang, {}).get("error", "❌ An error occurred"))
            await Mbot.send_message(LOG_GROUP, traceback.format_exc())
