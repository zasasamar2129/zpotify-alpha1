...
# Add this with other imports at the top of spotify.py
import shutil
...
from mbot.__init__ import (
    ...
    DAILY_LIMITS_FILE)
from mbot.utils.ytdownloader import (
    robust_download,
    DownloadError,
    thumb_down as robust_thumb_down
)
import logging
...
async def spotify_dl(Mbot,message: Message):
    msg = await message.reply("🚀 Processing your Spotify music...")
...
# Check maintenance mode
    ...
    'extractaudio': True,
    'audioformat': 'mp3',
    'outtmpl': f'{randomdir}/%(title)s.%(ext)s',  # This will be sanitized by our new function
    'quiet': True,
    'addmetadata': True,
    'postprocessors': [{
        ...
        'preferredquality': '320',
                ...
                path = await robust_download(f"ytsearch:{song['name']} {song['artist']}", download_opts)
            except Exception as e:
                pass
...
## optional you can clear this or add this by using #
  ...
  #     T = await message.reply_text(f"[{song.get('name')} - {song.get('artist')}](https://open.spotify.com/track/{song.get('deezer_id')}) Track Not Found ⚠️")
    ...
    #    await message.reply_text(f"try: `/saavn {song.get('name')}`")
        ...
        await sleep(2.0)
    try:
        if 'randomdir' in locals() and os.path.exists(randomdir):
            shutil.rmtree(randomdir, ignore_errors=True)
    except Exception as e:
        logger.error(f"Cleanup error: {str(e)}")
        if BUG:  # Only if BUG is defined
            await Mbot.send_message(BUG, f"Cleanup failed for {randomdir}: {str(e)}")
        try:
            await message.reply_text(
