...
from mbot.plugins.Shazam import *  # Add this import
from pyrogram.enums import ParseMode  # Required for parse mode constant
import os
...
async def settings(client, message):
    ...
    # Check premium status for button text
    is_premium = premium.is_premium(message.from_user.id)
    premium_button = "🌟 Premium Status" if is_premium else "🌟 Get Premium"
...
async def admin_panel(client, message):
    ...
    keyboard = [
        ...
        [
            InlineKeyboardButton("🍪 IG Cookie Management", callback_data="cookie_management"),
            InlineKeyboardButton("🤖 Manage Bot", callback_data="bot_management"),
        ],
        [
...
USER_LIST_FILE = os.path.join(DB_FOLDER, 'user_list.json')
...
async def settings_menu(client, callback_query):
    ...
    # Check premium status for button text
    is_premium = premium.is_premium(callback_query.from_user.id)
    premium_button = "🌟 Premium Status" if is_premium else "🌟 Get Premium"
...
async def view_profile(client, callback_query):
    ...
    # Get premium info
    premium_status = premium.get_premium_info(user.id)
    
    # Get daily usage info
    remaining, reset_time = premium.check_daily_limit(user.id)
    if remaining == float('inf'):
        daily_usage = responses['unlimited']
    else:
        reset_str = reset_time.strftime("%Y-%m-%d %H:%M") if reset_time else "soon"
        daily_usage = f"{NON_PREMIUM_DAILY_LIMIT - remaining}/{NON_PREMIUM_DAILY_LIMIT} (resets {reset_str})"
    
    # Get premium plan details if premium
    plan_info = ""
    if premium.is_premium(user.id):
        user_data = premium.premium_users.get(str(user.id), {})
        plan = user_data.get("plan", "standard")
        plan_info = f"\n{responses['premium_plan']}: {plan.capitalize()}"
    
    profile_text = (
        f"<b>{responses['profile_title']}</b>\n\n"
        f"{responses['user_id']}: <code>{user.id}</code>\n"
        f"{responses['first_name']}: {user.first_name}\n"
        f"{responses['last_name']}: {user.last_name or responses['not_set']}\n"
        f"{responses['username']}: @{user.username if user.username else responses['not_set']}\n"
        f"{responses['phone_number']}: {getattr(user, 'phone_number', responses['not_visible'])}\n"
        f"{responses['language']}: {LANGUAGES.get(user_lang, responses['not_set'])}\n"
        f"{premium_status}{plan_info}\n"
        f"{responses['daily_usage']}: {daily_usage}"
    )
    ...
    try:
        ...
        print(f"[⚠️] Error fetching profile photo: {e}")
...
@Mbot.on_callback_query(filters.regex("^premium_management$") & filters.user(SUDO_USERS))
async def premium_management(client, callback_query):
    keyboard = [
        [
            InlineKeyboardButton("➕ Add Premium User", callback_data="add_premium_user"),
            InlineKeyboardButton("➖ Remove Premium", callback_data="remove_premium_user"),
        ],
        [
            InlineKeyboardButton("👑 List Premium Users", callback_data="list_premium_users"),
            InlineKeyboardButton("⏱️ Set Default Cooldown", callback_data="set_default_cooldown"),
        ],
        [
...
@Mbot.on_callback_query(filters.regex("^add_premium_user$") & filters.user(SUDO_USERS))
async def add_premium_user(client, callback_query):
    await callback_query.edit_message_text(
        "➕ Add Premium User\n\n"
        "Please send the user ID and duration in days in this format:\n"
...
@Mbot.on_callback_query(filters.regex("^remove_premium_user$") & filters.user(SUDO_USERS))
async def remove_premium_user(client, callback_query):
    await callback_query.edit_message_text(
        "➖ Remove Premium User\n\n"
...
@Mbot.on_callback_query(filters.regex("^list_premium_users$") & filters.user(SUDO_USERS))
async def list_premium_users(client, callback_query):
    if not premium.premium_users:
...
@Mbot.on_callback_query(filters.regex("^set_default_cooldown$") & filters.user(SUDO_USERS))
async def set_default_cooldown(client, callback_query):
    await callback_query.edit_message_text(
        "⚙️ Set Default Cooldown\n\n"
        "Please send the cooldown time in seconds:\n"
        "<code>/setcooldown seconds</code>\n\n"
        "Example: <code>/setcooldown 30</code>",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🔙 Back", callback_data="premium_management")]
        ])
    )
...
@Mbot.on_callback_query(filters.regex("^back_to_admin$") & filters.user(SUDO_USERS))
...
# Add this after your other command handlers, before the admin panel section
@Mbot.on_message(filters.command("cookie_status") & filters.user(SUDO_USERS))
async def cookie_status(client, message):
    await message.delete()
    status = downloader.get_cookie_health()
    response = (
        "🍪 Cookie Status:\n"
        f"• Status: {status['status'].upper()}\n"
        f"• Age: {status['age_hours']:.1f} hours\n"
        f"• Last Refresh: {datetime.datetime.fromtimestamp(status['last_refresh'])}"
    )
    await message.reply(response)

@Mbot.on_message(filters.command("refresh_cookies") & filters.user(SUDO_USERS))
async def manual_refresh(client, message):
    await message.delete()
    await message.reply("🔄 Attempting manual cookie refresh...")
    if downloader.refresh_cookies():
        await message.reply("✅ Cookies refreshed successfully!")
    else:
        await message.reply("❌ Cookie refresh failed")

@Mbot.on_message(filters.command("test_cookies") & filters.user(SUDO_USERS))
...
@Mbot.on_message(filters.command("test_rapidapi") & filters.user(SUDO_USERS))
async def test_rapidapi(client, message):
    await message.delete()
    if not downloader.rapidapi_key:
        await message.reply("❌ RapidAPI key not configured")
        return
    
    try:
        headers = {
            'x-rapidapi-key': "0c523460bemsh5492eda6314db2fp14ec57jsn72b4bf7c01c8",
            'x-rapidapi-host': "instagram-api-fast-reliable-data-scraper.p.rapidapi.com"
        }
        querystring = {"id":"3565055737711082763"}
        test_url = "https://instagram-api-fast-reliable-data-scraper.p.rapidapi.com/media"
        ...
        if response.status_code == 200:
            await message.reply("✅ RapidAPI connection working")
        elif response.status_code == 403:
            await message.reply("❌ RapidAPI key invalid or expired")
        elif response.status_code == 429:
            await message.reply("⚠️ RapidAPI rate limit reached")
        else:
            await message.reply(f"❌ API returned status code: {response.status_code}")
            
    except requests.exceptions.RequestException as e:
        await message.reply(f"❌ API connection failed: {str(e)}")
    except Exception as e:
        await message.reply(f"❌ Unexpected error: {str(e)}")

# Add this new callback handler for cookie management
@Mbot.on_callback_query(filters.regex(r"cookie_management"))
async def cookie_management_panel(client, callback_query):
    await callback_query.answer()
    
    keyboard = [
        [
            InlineKeyboardButton("🍪 Cookie Status", callback_data="cookie_status"),
            InlineKeyboardButton("🔄 Refresh Cookies", callback_data="refresh_cookies"),
        ],
        [
            InlineKeyboardButton("🧪 Test Cookies", callback_data="test_cookies"),
            InlineKeyboardButton("⚡ Test RapidAPI", callback_data="test_rapidapi"),
        ],
        [
            InlineKeyboardButton("🔙 Back", callback_data="admin")
        ]
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await callback_query.message.edit_text("🍪 IG Cookie Management:\nChoose an action:", reply_markup=reply_markup)

# Add these callback handlers for the cookie management buttons
@Mbot.on_callback_query(filters.regex(r"cookie_status"))
async def cookie_status_callback(client, callback_query):
    await callback_query.answer()
    await cookie_status(client, callback_query.message)

@Mbot.on_callback_query(filters.regex(r"refresh_cookies"))
async def refresh_cookies_callback(client, callback_query):
    await callback_query.answer()
    await manual_refresh(client, callback_query.message)

@Mbot.on_callback_query(filters.regex(r"test_cookies"))
async def test_cookies_callback(client, callback_query):
    await callback_query.answer()
    await test_cookies(client, callback_query.message)

@Mbot.on_callback_query(filters.regex(r"test_rapidapi"))
async def test_rapidapi_callback(client, callback_query):
    await callback_query.answer()
    await test_rapidapi(client, callback_query.message)
